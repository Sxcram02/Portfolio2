    ...
    class Database {
        ...
        {
                $mySqlConnection = $this -> newObjMysql;
                $getQuery = "SELECT blg.titulo as titulo, blg.descripcion as descripcion FROM blog blg JOIN users usr ON blg.user = usr.clientName WHERE usr.clientName = 'marcos';";

                $result = $mySqlConnection -> query($getQuery);
                    echo "<article>";
                        foreach($arra as $dataPost){
                            echo "<img src='' alt='' />";
