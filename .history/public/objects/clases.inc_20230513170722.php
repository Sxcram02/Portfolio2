    ...
    class Database {
        ...
        {
                $mySqlConnection = $this -> newObjMysql;
                $getQuery = "SELECT blg.titulo as titulo, blg.descripcion as descripcion, blg.subtitulo as subtitulo FROM blog blg JOIN users usr ON blg.user = usr.clientName WHERE usr.clientName = 'marcos';";

                $result = $mySqlConnection -> query($getQuery);
                    echo "<article>";
                        foreach($result as $dataPost){
                            echo "<img src='{$dataPost['imagen']' alt='' />";
